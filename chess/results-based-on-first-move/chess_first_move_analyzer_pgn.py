...
def analyze_games(pgn_file):
    username = os.path.splitext(os.path.basename(pgn_file))[0]  # Extract username from filename
    white_openings = {}
    black_openings = {}
    stats = {
        'total': 0,
...
# Get a list of all PGN files in the current directory
pgn_files = [f for f in os.listdir('.') if f.endswith('.pgn')]

# If no PGN files are found, exit
if not pgn_files:
...
for pgn_file in pgn_files:
    white_openings, black_openings, game_stats = analyze_games(pgn_file)  # Get white_openings and black_openings
    print(f"\nAnalyzing PGN file: {pgn_file}")
    print_results(white_openings, black_openings, game_stats)  # Pass white_openings and black_openings
    print()
