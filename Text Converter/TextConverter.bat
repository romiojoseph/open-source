...
echo 3. Title Case:
echo %text% | powershell -command "$exceptions = @('a','an','the','and','but','or','nor','for','so','yet','at','by','for','in','of','on','to','with'); $words = $input -split '\s+'; for ($i = 0; $i -lt $words.Count; $i++) { if ($i -eq 0 -or $exceptions -notcontains $words[$i].ToLower()) { $words[$i] = (Get-Culture).TextInfo.ToTitleCase($words[$i].ToLower()) } else { $words[$i] = $words[$i].ToLower() } }; $words -join ' '"
echo.
